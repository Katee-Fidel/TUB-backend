...
async function getEventById(req, res) {
  ...
  } catch (_err) { return res.status(400).json({ message: 'Invalid event id' }); }
}
...
async function updateEvent(req, res) {
  try {
    ...
    for (const field of ['title', 'description', 'venue', 'date', 'ticketPrice', 'totalTickets', 'status']) if (req.body[field] !== undefined) event[field] = req.body[field];
    if (event.totalTickets < event.ticketsSold + event.reservedTickets) return res.status(400).json({ message: 'Total tickets cannot be less than sold or reserved tickets' });
    if (req.file) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (paymentMethod === 'wallet') {
        const currentWallet = await Wallet.findOneAndUpdate({ _id: wallet._id, balance: { $gte: totalAmount } }, { $inc: { balance: -totalAmount } }, { new: true, session });
        if (!currentWallet) { const error = new Error(`Insufficient wallet balance (need KES ${totalAmount})`); error.status = 400; throw error; }
      }
      ...
      ticket.qrData = qrData; ticket.qrCode = qrData;
      await ticket.save({ session });
      ...
      if (paymentMethod === 'wallet') {
        ...
        await createLedgerEntry({ user: req.user.id, wallet: wallet._id, event: event._id, ticket: ticket._id, type: 'ticket_purchase', direction: 'debit', amount: totalAmount, status: 'completed' }, session);
      } else {
        await createLedgerEntry({ user: req.user.id, wallet: wallet._id, event: event._id, ticket: ticket._id, type: 'ticket_purchase', direction: 'debit', amount: totalAmount, status: 'pending', phone: normalizedPhone }, session);
      }
    });
    await session.endSession(); session = null;

    if (paymentMethod === 'wallet') {
      try { ticket.qrImageUrl = await generateAndUploadQRCode(ticket.qrData, ticket._id.toString()); await ticket.save(); }
      catch (qrError) { console.error('Wallet ticket QR generation failed:', qrError); }
