...
async function getMyWallet(req, res) {
  try { const wallet = await Wallet.findOne({ user: req.user.id }).populate('savingGoals.event', 'title date bannerUrl'); if (!wallet) return res.status(404).json({ message: 'Wallet not found' }); return res.status(200).json({ wallet }); }
...
async function getWalletTransactions(req, res) {
  try { const wallet = await Wallet.findOne({ user: req.user.id }).select('_id'); if (!wallet) return res.status(404).json({ message: 'Wallet not found' }); const transactions = await Transaction.find({ wallet: wallet._id }).populate('event', 'title date').populate('ticket', 'quantity totalAmount status').sort({ createdAt: -1 }); return res.status(200).json({ transactions }); }
...
async function topupWallet(req, res) {
  try { const amount = Number(req.body.amount); const rawPhone = String(req.body.phone || '').trim(); if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ message: 'Top-up amount must be greater than 0' }); if (!rawPhone) return res.status(400).json({ message: 'Phone number is required' }); const normalizedPhone = rawPhone.replace(/^\+/, '').replace(/^0/, '254'); if (!/^254\d{9}$/.test(normalizedPhone)) return res.status(400).json({ message: 'Enter a valid phone number in the format 2547xx...' }); const wallet = await Wallet.findOne({ user: req.user.id }); if (!wallet) return res.status(404).json({ message: 'Wallet not found' }); const transaction = await createLedgerEntry({ user: req.user.id, wallet: wallet._id, type: 'wallet_topup', direction: 'credit', amount, phone: normalizedPhone, merchantRequestID: `pending-${Date.now()}`, checkoutRequestID: `pending-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`, status: 'pending' }); try { const response = await initiateStkPush({ phone: normalizedPhone, amount: Math.round(amount), accountReference: `TUB-${wallet._id.toString().slice(-6)}`, transactionDesc: 'Wallet top up' }); transaction.merchantRequestID = response.MerchantRequestID || transaction.merchantRequestID; transaction.checkoutRequestID = response.CheckoutRequestID || transaction.checkoutRequestID; transaction.resultDesc = response.ResponseDescription || 'STK push initiated'; await transaction.save(); return res.status(200).json({ message: 'Top-up request initiated', transaction }); } catch (error) { transaction.status = 'failed'; transaction.resultDesc = error.message || 'STK push failed'; await transaction.save(); return res.status(502).json({ message: 'Could not initiate M-Pesa request. Please try again.' }); } }
...
async function handleMpesaCallback(req, res) {
  try {
    ...
    if (transaction?.type === 'wallet_topup') {
      ...
      const wallet = await Wallet.findById(transaction.wallet); if (!wallet) { transaction.status = 'failed'; transaction.resultDesc = 'Wallet not found'; await transaction.save(); return res.status(200).json({ ResultCode: 0, ResultDesc: 'Accepted' }); }
      wallet.balance = Number(wallet.balance || 0) + callbackAmount; await wallet.save(); transaction.status = 'completed'; transaction.mpesaReceiptNumber = metadata.MpesaReceiptNumber ? String(metadata.MpesaReceiptNumber) : transaction.mpesaReceiptNumber; await transaction.save(); return res.status(200).json({ ResultCode: 0, ResultDesc: 'Accepted' });
    }
    ...
    const releaseReservation = async () => { await Event.updateOne({ _id: pendingTicket.event, reservedTickets: { $gte: pendingTicket.quantity } }, { $inc: { reservedTickets: -pendingTicket.quantity } }); };
    ...
    if (resultCode !== 0) {
      ...
      if (cancelled) { await releaseReservation(); if (transaction?.type === 'ticket_purchase') { transaction.status = 'failed'; transaction.resultCode = resultCode; transaction.resultDesc = resultDesc; transaction.merchantRequestID = merchantRequestID || transaction.merchantRequestID; await transaction.save(); } }
    ...
    if (!Number.isFinite(callbackAmount) || callbackAmount <= 0 || Math.abs(callbackAmount - Number(pendingTicket.totalAmount)) > 0.01) {
      const cancelled = await Ticket.findOneAndUpdate({ _id: pendingTicket._id, status: 'pending' }, { $set: { status: 'cancelled', merchantRequestID: merchantRequestID || pendingTicket.merchantRequestID } }, { new: true });
      if (cancelled) { await releaseReservation(); if (transaction?.type === 'ticket_purchase') { transaction.status = 'failed'; transaction.resultCode = resultCode; transaction.resultDesc = 'Invalid or mismatched callback amount'; transaction.merchantRequestID = merchantRequestID || transaction.merchantRequestID; await transaction.save(); } }
      return res.status(200).json({ ResultCode: 0, ResultDesc: 'Accepted' });
    }
    ...
    const sold = await Event.updateOne({ _id: paidTicket.event, reservedTickets: { $gte: paidTicket.quantity } }, { $inc: { reservedTickets: -paidTicket.quantity, ticketsSold: paidTicket.quantity } });
    if (!sold.modifiedCount) console.error('Could not finalize reserved ticket inventory', { ticketId: paidTicket._id.toString() });
    return res.status(200).json({ ResultCode: 0, ResultDesc: 'Accepted' });
