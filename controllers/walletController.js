...
async function getTopupStatus(req, res) {
  try {
    const transaction = await Transaction.findOne({ _id: req.params.id, user: req.user.id, type: 'wallet_topup' });
...
async function handleMpesaCallback(req, res) {
  try {
    ...
    // Wallet top-up callbacks are represented by ledger transactions.
    ...
    if (transaction) {
      ...
      transaction.mpesaReceiptNumber = metadata.MpesaReceiptNumber ? String(metadata.MpesaReceiptNumber) : transaction.mpesaReceiptNumber;
      await transaction.save();
      return res.status(200).json({ ResultCode: 0, ResultDesc: 'Accepted' });
    }

    // Preserve the current main ticket callback flow: atomically claim pending
    // tickets so repeated provider callbacks cannot increment ticketsSold twice.
